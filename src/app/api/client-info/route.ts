...
async function getLocationByIP(ip: string): Promise<Partial<ClientInfo>> {
  try {
    ...
    const locationData = {
      ip: data.query,
      city: data.city?.toLowerCase().trim() || undefined,
      region: data.regionName?.toLowerCase().trim() || undefined,
      regionCode: data.regionCode?.toLowerCase() || undefined,
      country: data.country?.toLowerCase() || undefined,
      countryCode: data.countryCode?.toLowerCase() || undefined,
      postalCode: data.zip?.replace(/\D/g, '') || undefined,
...
async function getLocationByIPBackup1(ip: string): Promise<Partial<ClientInfo>> {
  try {
    ...
    const locationData = {
      ip: ip,
      city: data.city?.toLowerCase().trim() || undefined,
      region: data.state_prov?.toLowerCase().trim() || undefined,
      regionCode: data.state_prov?.toLowerCase().trim() || undefined,
      country: data.country_name?.toLowerCase() || undefined,
      countryCode: data.country_code2?.toLowerCase() || undefined,
      postalCode: data.zipcode?.replace(/\D/g, '') || undefined,
...
async function getLocationByIPBackup2(ip: string): Promise<Partial<ClientInfo>> {
  try {
    ...
    const locationData = {
      ip: ip,
      city: data.city?.toLowerCase().trim() || undefined,
      region: data.region?.toLowerCase().trim() || undefined,
      regionCode: data.region_code?.toLowerCase() || undefined,
      country: data.country_name?.toLowerCase() || undefined,
      countryCode: data.country_code?.toLowerCase() || undefined,
      postalCode: data.postal?.replace(/\D/g, '') || undefined,
      timezone: data.timezone,
      isp: data.org,
      org: data.org,
      lat: data.latitude,
...
export async function GET(request: Request) {
  try {
    ...
    const clientInfo: ClientInfo = {
      ...
      isp: locationData.isp || undefined,
      org: locationData.org || undefined,
      as: locationData.as || undefined,
